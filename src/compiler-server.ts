...
export function resolve(rootDir: string) {
      ...
      if (url.match(/\/module\.(j|t)sx?$/)) {
        ...
        if (src) {
          ...
          const module = ts.transpileModule(content, {
            compilerOptions,
            transformers: {
              before: [importTransformer as never]
            }
          }).outputText
      ...
      if (
        ...
        if (modulePath.match(/@|#/)) {
          const moduleSpecifier = ts.factory.createStringLiteral(
            modulePath.replace(/^@/, '/SCOPED_MODULE:').replace(/^#/, '/SUBPATH_MODULE:')
          )
...
function resolveModuleName(moduleName: string, containingFile: string) {
  return ts.resolveModuleName(
    ...
    },
    {
      ...
      // eslint-disable-next-line @typescript-eslint/unbound-method
      realpath: ts.sys.realpath as never
    }
